...
- **Shutdown hazard found on the way** (finding 5): programs with no
  ...
  aborts. Repro is `sleepsort-not.lind` itself (a control with an
  explicit `Error { }` exits cleanly); candidate fixes and the
  full autopsy in the README. Filed as §11.12.
